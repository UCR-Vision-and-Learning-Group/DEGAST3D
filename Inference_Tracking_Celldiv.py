...
save_dir_label = f'results/label/{seg_method}_{target_plant}'
save_dir_vis = f'results/visual/{seg_method}_{target_plant}'
...
for t1 in tqdm(range(start,n-d)):
    ...
    np.save(f'{save_dir_label}/all_tracks_stack_{seg_files[t1]}_{seg_files[t1+d]}.npy',all_tracks)
    np.save(f'{save_dir_label}/all_divisions_stack_{seg_files[t1]}_{seg_files[t1+d]}.npy',all_div_info)



    # Visualization
    visualize_and_save(seg0,seg1,all_tracks[:,0],all_tracks[:,1],f'{save_dir_visual}/output_time_pair_{t1}_{seg_files[t1]}',f'{save_dir_visual}/output_time_pair_{t1}_{seg_files[t1+d]}')